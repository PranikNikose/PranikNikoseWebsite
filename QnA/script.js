...
const utils = {
  generateId(list) {
    return list.length ? Math.max(...list.map(n => n.id)) + 1 : 1;
  },
...
const notes = {
  init() {
    ...
    if (!stored || !stored.length) storage.save(state.notes);
  },
  ...
  add(payload) {
    const id = utils.generateId(state.notes);
  ...
  duplicate(id) {
    ...
    const copy = {
      ...source,
      id: utils.generateId(state.notes),
